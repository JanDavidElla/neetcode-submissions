    ...
    def searchMatrix(self, matrix: List[List[int]], target: int) -> bool:
        ...
        while left < right:
            ...
            if matrix[mid][0] == target:
                ...
                if matrix[mid+1][0] > target: #edge case: before shifting right, it checks the rightest matrix's first element.
                                                #if it is greater, then we can assume, mid is the one likely with the element.
                    left = mid
                    break
                left = mid + 1
            else:
